...
PUSH_BOOL(ret)
END_IMGUI_FUNC
//    IMGUI_API bool          ButtonEx(const char* label, const ImVec2& size = ImVec2 0 0, ImGuiButtonFlags flags);            // button
IMGUI_FUNCTION(ButtonEx)
LABEL_ARG(label)
  OPTIONAL_IM_VEC_2_ARG(size, 0, 0)
OPTIONAL_INT_ARG(flags, 0)
CALL_FUNCTION(ButtonEx, bool, label, size, flags)
PUSH_BOOL(ret)
END_IMGUI_FUNC
...
OPTIONAL_INT_ARG(decimal_precision, -1)
OPTIONAL_INT_ARG(extra_flags, 0)
CALL_FUNCTION(InputFloat2, bool, label, x, y, decimal_precision, extra_flags)
PUSH_BOOL(ret)
END_FLOAT_POINTER(x)
END_FLOAT_POINTER(y)
END_IMGUI_FUNC
